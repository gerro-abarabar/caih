...
def ask_llm(question):
    ...
    for _ in range(max_retries):
        try:
            message = llm_message[choice - 1].format(
...
def handle_data_question(question):
    ...
    while (
        not explanation
        or explanation.strip() == "No explanation available after multiple attempts."
    ):
...
def process_question(model, question):
    ...
    try:
        ...
        if (
            ...
            == "No explanation available after multiple attempts."
        ):
            # 1. Get LLM result
...
if __name__ == "__main__":
    ...
    model_used = ["gemma4:31b-cloud", "ministral-3:14b-cloud", "qwen3.5:397b-cloud"][
        choice - 1
    ]
    print(model_used)
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for i in range(0, len(exam), batch_size):
            ...
            final_data.extend(
                wait_until_complete(
                    batch=len(batch), original_images=original["images"]
                )
            )
            print(final_data)
    server_socket.close()
